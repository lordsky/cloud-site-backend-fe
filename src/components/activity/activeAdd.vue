<template>
  <div>
    <el-form ref="active" :model="active" :rules="rules" label-width="100px" class="activeAdd">
      <el-form-item label="active名称:" prop="name">
        <el-input v-model="active.name" placeholder="请输入套件标题，不超过12个字符" class="el-input-active"></el-input>
      </el-form-item>
      <el-form-item label="active位置:" prop="classification" v-if="pageId != 2">
        <el-select v-model="active.classification" placeholder="请选择套件分类" class="el-select-active">
          <el-option :label="x.catName" :value="i" v-for="(x,i) in classification" :key="i"></el-option>
        </el-select>
      </el-form-item>
      <el-form-item label="active位置:" v-if="pageId == 2">
        <el-select v-model="catType.catName" placeholder="请选择模版分类" class="el-select-active" disabled>
          <el-option  :key="catType.id" :label="catType.catName" :value="catType.catName">
          </el-option>
        </el-select>
      </el-form-item>
      <el-form-item label="active链接:" prop="activeLink">
        <el-radio-group v-model="activeLink">
        ...
        <div v-if="activeLink == 1">
          ...
          <div>注：选择活动链接，点击active时将跳转到活动详情页</div>
        </div>
        <div v-if="activeLink == 2">
          <el-input v-model="active.name" placeholder="请输入外部链接地址" class="el-input-active"></el-input>
        </div>
      </el-form-item>
      <el-form-item label="active图片:">
        <div class="footerside-right-list" @mousemove="showDel = true" @mouseleave="showDel=false">
          <!--<img v-if="active.imageUrl" :src="active.imageUrl" class="avatar">-->
          <div v-if="active.imageUrl" :class="{'delItem':showDel}">
        ...
        <el-dialog :visible.sync="dialogVisible">
          <img width="100%" :src="active.imageUrl" alt="">
        </el-dialog>
      </el-form-item>
      <el-form-item label="状态:" prop="activeLink">
